...
# Load datasets
    ...
    # Save the top candidates to the PostgreSQL database for each job role
    # for role, candidates_df in top_candidates_per_role.items():
        # role_with_underscore = role.replace(' ', '_')
        # table_name = f'candidates_for_{role_with_underscore}'
        # save_to_database(candidates_df, table_name)
        # # table_name = f'candidates_for_{role}'
        # # save_to_database(candidates_df, table_name)
